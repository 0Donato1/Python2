...
class Girlfriend:
    rand = {
        'Restaurant': {'money_spend': 50, 'gladness_up': 50},
        'Park': {'money_spend': 30, 'gladness_up': 25},
    }
    def __init__(self):
        self.activity = random.choice(list(Girlfriend.rand))
        self.money_spend = Girlfriend.rand[self.activity]['money_spend']
        self.gladness_up = Girlfriend.rand[self.activity]['gladness_up']
...
class Human:
    ...
    def is_alive(self):
        if self.gladness < 0:
            ...
            print('Bankrupt...')
            return False
        rastovanie = random.randint(1, 200)
        if rastovanie == 5:
            print('Breaking up with a girl. Depression...')
            return False
        else:
            return True
...
with open("simulation_log.txt", "w") as log_file:
    log_file.write(f"{nick.name} lived for {simulated_days} days in the simulation.")
